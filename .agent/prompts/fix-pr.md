...
Post a comment summarizing what was fixed. Read the pre-formatted usage footer from `.pi-usage-footer.txt` and append it to the comment.


```bash
# Write the main body to a temp file
...
sed -i "s|REQUESTER|${COMMENT_AUTHOR}|g; s|FIX_DESC|$2|g" /tmp/pi-comment.md
...
sops exec-env .agent/secrets.enc.yaml gh pr comment "$1" --body-file /tmp/pi-comment.md
```
### Step 8 — Cleanup
